...
import EpisodeInteractions from "./EpisodeInteractions";
...
export default function EngagementSection({
  ...
  const shareUrl = `${siteUrl}/stories/episode/${episodeSlug}`;

  return (
    <div className="space-y-6 min-w-0">
      {/* Engagement Bar */}
      <div className="py-6 border-y border-gray-200 dark:border-gray-800 min-w-0">
        {/* Desktop: flex row with justify-between, Mobile: flex-col centered */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-0">
          ...
          {/* Share Buttons */}
          <div className="flex items-center gap-2 flex-wrap min-w-0 justify-center sm:justify-end">
            <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">Share:</span>
            <a
            ...
            <a
              href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(`Read "${episodeTitle}" on ${siteName}`)}`}
              target="_blank"
              rel="noopener noreferrer"
            ...
            <a
              href={`https://wa.me/?text=${encodeURIComponent(`Read "${episodeTitle}" on ${siteName} ${shareUrl}`)}`}
              target="_blank"
              rel="noopener noreferrer"
            ...
            <ShareButton
              url={shareUrl}
              title={episodeTitle}
              text={episodeExcerpt || `Read "${episodeTitle}" on ${siteName}`}
            />
          </div>
        </div>
      </div>
